...
resolve_current_principal_object_id() {
  ...
  [[ -n "${object_id}" ]] || die "unable to resolve current Azure principal object id for the Anyscale Platform RBAC check"
  printf '%s' "${object_id}"
}
...
check_anyscale_platform_current_principal_rbac() {
  ...
  if [[ "${enabled}" != "true" ]]; then
    skip "M4-04 current-principal Anyscale Platform RBAC check disabled"
    return 0
  fi
...
check_m3() {
  ...
  local vm_state vm_power flex_nodes ready_flex_nodes artifact_dir anyscale_host_name host_runtime_json host_runtime_output
  ...
  az aks get-credentials --resource-group "${RG}" --name "${CLUSTER}" --overwrite-existing --only-show-errors >/dev/null
  artifact_dir="${STATE_DIR}/m3-flex-network"
  anyscale_host_name="$(lab_gate_anyscale_host_name)"

  mkdir -p "${artifact_dir}"
  host_runtime_json="${artifact_dir}/flex-host-runtime.json"
  # shellcheck disable=SC2016 # Expand these values on the Flex VM.
  az vm run-command invoke \
    --resource-group "${RG}" \
    --name "${FLEX_VM}" \
    --command-id RunShellScript \
    --scripts '. /etc/os-release; printf "ID=%s\nVERSION_ID=%s\nKERNEL=%s\n" "$ID" "$VERSION_ID" "$(uname -r)"' \
    --output json >"${host_runtime_json}"
  host_runtime_output="$(jq -r '.value[0].message // empty' "${host_runtime_json}")"
  grep -q '^ID=ubuntu$' <<<"${host_runtime_output}" || die "M3-05 Flex host must run Ubuntu; details: ${host_runtime_json}"
  grep -q '^VERSION_ID=24\.04$' <<<"${host_runtime_output}" || die "M3-05 Flex host must run the upstream Cilium lab's Ubuntu 24.04 image; details: ${host_runtime_json}"
...
check_m4() {
  ...
  if [[ "${wait_interval}" -lt 1 ]]; then
    die "M4 check has invalid M4_EXTENSION_WAIT_INTERVAL=${wait_interval}"
  fi
...
check_m6_local() {
  print_section "Module 6 local workload checks"

  local summary_path
  summary_path="${ROOT_DIR}/.cache/workloads/deepspeed_finetune/smoke/workload-summary.json"
  [[ -f "${summary_path}" ]] || die "M6-01 missing local smoke workload summary: ${summary_path}"
  pass "M6-01 local workload summary exists"

  python3 "${ROOT_DIR}/workloads/deepspeed_finetune/validate_workload_summary.py" "${summary_path}" >/dev/null
  pass "M6-02 local workload summary validates"
}

check_m7() {
  print_section "Module 7 cleanup checks"

  [[ "$(az group exists --name "${RG}")" == "false" ]] || die "M7-01 resource group still exists: ${RG}"
  pass "M7-01 resource group deleted"

  [[ "$(az group exists --name "${AKS_MANAGED_RG}")" == "false" ]] || die "M7-02 AKS managed resource group still exists: ${AKS_MANAGED_RG}"
  pass "M7-02 AKS managed resource group deleted"

  local state_output
  if ! state_output="$(cd "${ROOT_DIR}/infra/terraform" && terraform state list 2>/dev/null)"; then
    die "M7-03 unable to read Terraform state"
  fi
  [[ -z "${state_output}" ]] || die "M7-03 terraform state not empty"
  pass "M7-03 terraform state empty"
}

usage() {
  cat <<'EOF'
Usage: ./scripts/validate-lab.sh <phase>

Phases:
  m2          Check the live Module 2 environment.
  m3          Check the live Module 3 environment.
  m4          Check the live Module 4 environment.
  m5          Check the live Module 5 environment.
  m6-local    Check the local Module 6 workload result.
  preflight   Check readiness for Modules 2, 3, 4, and 5.
  teardown    Confirm Module 7 cleanup.
EOF
}
