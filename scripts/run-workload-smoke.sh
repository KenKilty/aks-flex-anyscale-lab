...
ensure_local_venv() {
  if [[ ! -x "${VENV_DIR}/bin/python" ]]; then
    printf 'Creating local smoke environment at %s\n' "${VENV_DIR}"
    python3 -m venv "${VENV_DIR}"
  fi

  printf 'Installing local smoke dependencies (the first run can take several minutes)\n'
...
printf 'Running the CPU smoke workload\n'
"${VENV_DIR}/bin/python" workloads/deepspeed_finetune/train.py \
  --profile smoke \
  --cpu-only \
  --local-smoke \
  --zero-stage 0 \
  --results-dir "${ROOT_DIR}/.cache/workloads/deepspeed_finetune/smoke"
