...
collect_kubernetes_placement_results() {
  ...
  if [[ "${mode}" == "cpu" || "${mode}" == "gpu" ]]; then
    jq -e --arg region "${TF_VAR_flex_region}" \
      ...
      "${placement_file}" >/dev/null || die "no ${mode} worker pod placed in Flex region ${TF_VAR_flex_region} (placement: ${placement_file})"
    jq -e --arg pool "${AKS_FLEX_AGENT_POOL_NAME}" \
      '.pods[] | select(.ray_node_type == "worker" and .node_agentpool == $pool)' \
      "${placement_file}" >/dev/null || die "no ${mode} worker pod placed on Flex agent pool ${AKS_FLEX_AGENT_POOL_NAME} (placement: ${placement_file})"
  fi
