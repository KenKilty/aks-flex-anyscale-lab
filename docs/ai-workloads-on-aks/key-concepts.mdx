...
| Ray worker | Runs the training function. There can be many. | CPU mode uses one worker on the Flex node. GPU mode uses one worker on an AKS-managed GPU node and one on the Flex GPU node. |

Both workload paths therefore use AKS-managed and Flex compute. In CPU mode, the
Ray head runs on an AKS-managed CPU node and the single training worker runs on
the Flex node. In GPU mode, the head remains on the managed CPU node while two
training workers run, one on each GPU location.
...
process that runs the training function. Its `ScalingConfig` starts one worker
for the CPU run and two workers for the GPU run in this lab.

Each worker has a **world rank**, a unique number within that training run, and
the **world size** is the total number of workers. The GPU run has world size 2,
...
Kubernetes pod placement, which proves that the processes ran on the two
different GPU nodes.
...
that the CPU worker ran on Flex, or that the two GPU workers ran concurrently on
the AKS-managed and Flex GPU nodes. Together, those records demonstrate one Ray
training workload using compute in two Azure regions.
:::
...
The diagram shows both possible worker locations. CPU mode uses the Flex worker;
GPU mode uses the AKS-managed and Flex workers together.
...
requirements and node selector to an eligible node. CPU mode places its training
worker on Flex. GPU mode places one worker on the AKS-managed GPU node and the
other on the Flex GPU node in the second region.
