...
process that runs the training function. Its
[`ScalingConfig`](https://docs.ray.io/en/latest/train/api/doc/ray.train.ScalingConfig.html)
starts two workers for both CPU and GPU runs in this lab.

Each worker has a
[**world rank**](https://docs.ray.io/en/latest/train/api/doc/ray.train.TrainContext.get_world_rank.html),
a unique number within that training run, and the
[**world size**](https://docs.ray.io/en/latest/train/api/doc/ray.train.TrainContext.get_world_size.html)
is the total number of workers. Both modes have world size 2,
...
[Kubernetes pod placement](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/),
which proves that the processes ran in the two regions.
...
For the training job, the script creates a tiny GPT-2-style model and feeds it
synthetic token sequences. A token is a piece of text, such as a word or part of
a word, that the model reads as a number. These sequences are randomly generated,
so they carry no meaning, but they match the length and numeric form the model
expects. They are the training equivalent of a crash-test dummy: the right size
and shape to exercise the system, without being a real passenger. The script
sends those batches to the Ray Train workers.

Each worker performs real training steps: the model predicts the
...
Unlike a real-world fine-tuning project, this lab does not start with a pretrained
...
model checkpoints. This lab uses a small model, synthetic test data, and four
...
:::info How you will prove the workload ran in two regions
...
Traditionally, an AKS cluster and all of its nodes come from AKS managed node
pools. Azure creates those virtual machines, joins them to the cluster, and
manages their lifecycle for you. That model works well when the capacity you need
is capacity Azure can provision in the cluster's own region. It also carries a
limitation: when you already have compute somewhere else, such as GPU servers in
your own datacenter, machines in another cloud, or spare capacity in a different
Azure region, AKS has not been able to use those machines as cluster nodes.

[**AKS Flex Node**](https://github.com/Azure/AKSFlexNode) removes that
limitation. It lets a Linux virtual machine or bare-metal host **outside**
...
A Ray application is the Python program you write, and it uses Ray to spread its
work across the cluster. A Job is one managed run of that program, from the
moment you submit it to the state it finishes in. In this lab, the application is the
...
[Anyscale on Azure networking](https://learn.microsoft.com/en-us/azure/anyscale-on-azure/networking)
