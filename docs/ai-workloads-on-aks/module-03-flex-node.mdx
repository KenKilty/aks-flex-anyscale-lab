...
`networkPlugin=none`, and [Unbounded](https://github.com/Azure/unbounded) owns
the networking path on both the managed AKS nodes and the Flex node.
...
AKS Flex Node prepares the Flex node VM and joins it to AKS as a worker. It
...
runs [kube-proxy](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-proxy/)
only on Flex, and links the managed AKS and Flex sites over the peered VNets.
Unbounded discovers the managed AKS node pod `/24`s and the Flex pod range, then
carries pod traffic between the nodes' private IP addresses.
...
| Kubernetes names and services | Flex pods use the normal [`ClusterFirst` DNS policy](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy). Unbounded-managed kube-proxy handles [`ClusterIP` Service](https://kubernetes.io/docs/concepts/services-networking/service/#type-clusterip) traffic on Flex, while the managed AKS path uses the same Unbounded ownership model. |
...
`TF_VAR_aks_pod_cidr`. `TF_VAR_unbounded_flex_pod_cidr` is the separate Flex
...
matching configuration tool, creates a
...
[certificate request](https://kubernetes.io/docs/reference/access-authn-authz/certificate-signing-requests/)
so the cluster trusts it.

A [taint](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/)
with the `NoSchedule` effect marks the node as reserved. Ordinary pods will not
land on it unless they have a matching toleration, which keeps unrelated
workloads off the Flex node you provisioned for this lab.

On a newly created Ubuntu VM, the command waits for
