...
take several minutes. The wrapper prints each phase before the workload starts.
...
The first GPU run can spend several minutes pulling the CUDA Ray image, so the
submission command allows more startup time for GPU workers than CPU workers.
...
## Step 8: Review the workload in the Anyscale console

The command-line results confirm placement. The Anyscale Console shows the same
run from the service side: job state, cluster shape, application logs, and live
