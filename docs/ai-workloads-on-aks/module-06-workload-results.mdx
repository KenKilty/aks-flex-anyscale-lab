...
description: Submit a two-worker Ray Train workload through Anyscale and prove that one worker trained on managed AKS while another trained on Flex.
sidebar_position: 8
tags:
  - anyscale
  - ray
...
**Region A**. Both modes run one training worker on managed AKS in Region A and
one on the Flex node in **Region B**. CPU mode uses CPU workers, while GPU mode
uses GPU workers. The command joins per-rank training records with Kubernetes
placement data so you can prove that both regions participated in the same
two-rank world.
...
and watch the Ray head and two worker pods land on different parts of the
cluster. You will inspect the saved training and placement records instead of
relying only on the Anyscale job status or a utilization chart.
...
places the Ray pods. Both modes create one fixed worker group on managed AKS and
one on Flex. Ray Train uses a spread
[placement strategy](https://docs.ray.io/en/latest/ray-core/scheduling/placement-group.html)
so the two training
actors run on different worker nodes. CPU mode selects CPU nodes, while GPU mode
selects GPU nodes. Both modes use the same two-worker shape, sizing code, and
...
sequenceDiagram
  ...
  participant managed as Ray worker on managed AKS
  ...
  alt CPU mode
    api-->>managed: Schedule one CPU worker in Region A
  ...
  command-->>learner: Save training results and placement evidence
```
...
| Workers | One managed AKS CPU worker and one Flex CPU worker | One managed AKS GPU worker and one Flex GPU worker |
...
must use the normal [`ClusterFirst` DNS policy](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy),
and the Flex node must not carry the broad `kubernetes.azure.com/cluster` label.
...
which runs 1 epoch and 4 training steps. Both modes start two workers, one on
managed AKS in Region A and one on Flex in Region B.
...
The live CPU run printed the following output. Your Gateway address, compute
config version, job name, and elapsed times will differ:

```text
PASS Gateway anyscale-operator/anyscale-gateway programmed at 20.80.176.35
info: Ray head uses CPU=2 memory=8Gi from live capacity on agentpool=cpu
info: managed CPU worker uses CPU=2 memory=8Gi from live capacity on agentpool=cpu
info: Flex CPU worker uses CPU=2 memory=8Gi from live capacity on agentpool=aksflexnodes
(anyscale +1.3s) Created compute config: 'cpu-home:4'
info: submit attempt 1/3 for flex-results-cpu-20260814-150555
(anyscale +2m34.0s) Job 'flex-results-cpu-20260814-150555' transitioned from STARTING to RUNNING
(anyscale +4m46.2s) Job 'flex-results-cpu-20260814-150555' transitioned from RUNNING to SUCCEEDED
(anyscale +4m46.2s) Job 'flex-results-cpu-20260814-150555' reached target state, exiting
```

**Expected CPU placement:** The CPU run places the Ray head and one training
worker on AKS CPU capacity in Region A, and the second training worker on the
Flex node in Region B. Both worker groups use `min_nodes=1` and `max_nodes=1`,
so Ray Train starts only after both locations are available. The workers use
...
Copy the generated job name from the submission output into `JOB_NAME`. You will
compare the generated compute config against live capacity, then inspect
Kubernetes placement, Anyscale state, and driver logs.

Start with the compute config. Set `CONFIG_NAME` to the mode you submitted. The
YAML should use CPU and memory values no larger than the selected nodes can fit
after headroom. A larger SKU can keep the workload target, while a smaller SKU
produces a lower request.

```bash
# CPU mode uses the line below. For GPU mode, use CONFIG_NAME="gpu-dual-home".
CONFIG_NAME="cpu-home"
...
info: managed CPU worker uses CPU=<derived-cpu> memory=<derived-memory>Gi from live capacity on agentpool=cpu
...
For either mode, run this table while the job is `STARTING` or `RUNNING`. Rerun
it until both named worker groups show `Running`. The compute config requires one
...
The live CPU placement table showed both fixed groups. Pod and node suffixes will
differ:

```text
WORKER              STATUS    NODE                                  GPU
managed-cpu-worker  Running   aks-cpu-13577184-vmss000001           0
flex-cpu-worker     Running   vm-flex-flexany-e2e814-scus           0
```
...
Expected GPU pod placement from `kubectl get pods`:
...
Once the run finishes, `anyscale job status` reports the final state:

```json
"state": "SUCCEEDED"
```

The driver logs carry the summary record that Step 6 saves to disk:
...
The Anyscale workload managed identity needs
[`Storage Blob Data Contributor`](https://learn.microsoft.com/en-us/azure/role-based-access-control/built-in-roles/storage)
on the
...
Expected CPU `workload-summary.json` shape. GPU mode has the same two ranks with
`cuda_available=true` and the selected GPU device names:
...
```json
{
  ...
  "worker_training_records": [
    {"rank": 0, "cuda_available": false, "device_name": "cpu", "steps_per_worker": 4, "loss": 10.8},
    {"rank": 1, "cuda_available": false, "device_name": "cpu", "steps_per_worker": 4, "loss": 10.8}
  ],
  "runtime_versions": {
  ...
  "storage_result": {
    ...
    "path": "<container-name>/anyscale/results/<job-name>/workload-summary.json",
    "uri": "az://<container-name>/anyscale/results/<job-name>/workload-summary.json"
  }
}
```

## Step 7: Prove both nodes trained

For CPU mode, verify that both training worker pods appear in the placement file:
...
jq -r '.pods[] | select(.ray_node_type == "worker") | [.node_name, .node_region, .node_agentpool] | @tsv' "$PLACEMENT" | column -t -s $'\t'

jq -e \
  --arg home "$TF_VAR_azure_location" \
  --arg flex "$TF_VAR_flex_region" '
    ([.pods[] | select(.ray_node_type == "worker" and .node_region == $home and .node_agentpool == "cpu")] | length) == 1 and
    ([.pods[] | select(.ray_node_type == "worker" and .node_region == $flex and .node_agentpool == "aksflexnodes")] | length) == 1
  ' "$PLACEMENT"

jq -n --slurpfile summary "$OUT" --slurpfile placement "$PLACEMENT" '
  ($placement[0].pods | map(select(.ray_node_type == "worker"))) as $pods
  | [$summary[0].worker_training_records[] as $record
    | ($pods[] | select(.name == $record.hostname)) as $pod
    | {rank: $record.rank, hostname: $record.hostname, node: $pod.node_name,
       region: $pod.node_region, pool: $pod.node_agentpool,
       steps: $record.steps_per_worker, loss: $record.loss}]
'
```

The `jq -e` command prints `true` only when one CPU worker pod ran on managed AKS
in Region A and the other ran on Flex in Region B. The final command joins the
training rank hostnames to those pods.

The live rank join printed the following result. Job names, pod names, and loss
...
[
  {
    "rank": 0,
    "hostname": "k-78c5865d8ed820000",
    "node": "aks-cpu-13577184-vmss000001",
    "region": "westus2",
    "pool": "cpu",
    "steps": 4,
    "loss": 10.855746030807495
  },
  {
    "rank": 1,
    "hostname": "k-e5c7c2ece96dd0000",
    "node": "vm-flex-flexany-e2e814-scus",
    "region": "southcentralus",
    "pool": "aksflexnodes",
    "steps": 4,
    "loss": 10.848628997802734
  }
]
```

The two distinct hostnames prove that the managed and Flex worker pods both
trained. Two Running worker pods are not sufficient evidence if both rank
records name the same pod.
...
The required Module 6 proof is complete after Step 7 joins each training rank to
one managed AKS pod and one Flex pod. For the GPU path, it also shows both CUDA
ranks and concurrent placement. You can finish the module without screenshots.
The optional Anyscale Console tour supplements those files with job state,
cluster shape, application logs, and live metrics.
...
## Validate the workload results

The CPU run is complete when the result shows `state=SUCCEEDED`, two training
records, one managed AKS CPU worker in Region A, and one Flex CPU worker in
Region B.

The GPU run is complete when the result shows `state=SUCCEEDED`, two CUDA
training records, two worker pods running at the same time, and one worker on
the managed GPU pool with the other on `agentpool=aksflexnodes`.
