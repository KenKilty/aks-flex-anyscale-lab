...
places the Ray pods. CPU mode creates one Flex worker. GPU mode creates one fixed
worker group for the managed AKS GPU and one for the Flex GPU. Both modes use
the same sizing code and Unbounded network path.
