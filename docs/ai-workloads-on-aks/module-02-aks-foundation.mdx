...
before it starts the Flex VM. The cluster uses `networkPlugin=none`, and
Terraform adds Unbounded to provide pod networking for the managed AKS nodes.
Terraform also gives the Flex VM's managed identity permission to register its
machine with AKS. Module 3 extends the pod network to the Flex node, using
