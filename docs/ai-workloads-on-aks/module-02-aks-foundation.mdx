...
- An **Azure Container Registry (ACR)** that Anyscale registers as the cloud's
  image registry and AKS is authorized to pull images from.
- An **Azure Storage account** that Anyscale uses for cloud storage. Later in
  Module 6, the training job also writes its result summary there.
- **Log Analytics and Container Insights** to collect AKS container logs,
  metrics, and Azure resource diagnostics.
- Two **peered virtual networks (VNets)** that give the AKS nodes and Flex VM a
