...
```mermaid
flowchart LR
  regionA["Region A<br/>AKS cluster and Ray head<br/>Azure CNI Overlay with managed Cilium"]
  network["Peered Azure networks<br/>Unbounded SitePeering carries pod traffic"]
  regionB["Region B<br/>Flex node and Ray worker<br/>Unbounded CNI and kube-proxy"]
...
the host's connection to the cluster. Azure CNI Overlay powered by managed
Cilium owns pod networking on AKS nodes, while Unbounded owns CNI and
kube-proxy on the Flex node. Unbounded SitePeering carries pod traffic between
the two sites over the peered VNets.
...
pods on the matching nodes, and the mixed-CNI path carries traffic between them
over the peered VNets. The workload saves its results in
`workload-summary.json`. After
...
Start with **[Module 1: Prepare Your Environment](./module-01-environment-setup)**.
It validates your workstation and subscription without creating Azure resources;
the first deployment happens in Module 2.
