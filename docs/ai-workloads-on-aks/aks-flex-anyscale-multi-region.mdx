...
job submission in one place while Kubernetes places the Ray head and worker pods
where the cluster has capacity.
...
`networkPlugin=none`, and Unbounded provides pod networking for the managed AKS
nodes and the Flex VM. Module 2 creates a peered Azure VNet for the Flex host,
then the later modules verify that the workload spans both node pools.
...
Behind that flow, Anyscale sends your workload to the Anyscale operator running
in AKS. The operator creates the Ray resources, Kubernetes places the pods on the
