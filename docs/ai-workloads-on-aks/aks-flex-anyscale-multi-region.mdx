...
places the [Ray](https://github.com/ray-project/ray) head and worker pods where
the cluster has capacity.

You will use **AKS Flex Node** to join a Linux host outside the AKS managed node
pools as a worker node. **Anyscale on Azure** adds managed
[Ray Jobs](https://docs.ray.io/en/latest/cluster/running-applications/job-submission/index.html),
compute configs, and workload observability. Together, they let you submit one
job while Kubernetes places its parts across AKS and Flex.

The lab runs a Ray Train and DeepSpeed fine-tuning demonstration across two
locations. The Ray head runs on an AKS CPU node in Region A. Both modes run one
training rank on managed AKS in Region A and another on Flex in Region B. CPU
mode uses CPU workers, while GPU mode uses GPU workers. You will compare
per-rank training records with Kubernetes placement.
...
In this setup, **Region A** hosts the AKS cluster, Ray head pod, and managed CPU
or GPU worker. A
reachable Linux host in **Region B** joins that same cluster as an AKS Flex node
and runs the second CPU or GPU worker pod. **Anyscale on Azure** submits and observes the
...
```mermaid
flowchart LR
  regionA["Region A<br/>AKS head and managed CPU or GPU worker<br/>Unbounded pod networking"]
  network["Peered Azure networks<br/>Unbounded carries pod traffic"]
  regionB["Region B<br/>Flex node and second CPU or GPU worker<br/>Unbounded pod networking + kube-proxy"]
...
| [2: Deploy AKS and Azure Resources](./module-02-aks-foundation) | Create the AKS cluster, storage, a container registry, the Flex VM, and the cross-region networking between them. |
