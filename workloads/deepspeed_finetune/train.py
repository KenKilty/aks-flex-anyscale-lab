...
"""Representative GPU workload for the AKS Flex sample.

This is a script-first adaptation of the Anyscale deepspeed_finetune template.
It defaults to synthetic inputs so the run is repeatable and does not depend
...
class LocalSmokeEngine:
    def __init__(self, model: Any, optimizer: torch.optim.Optimizer) -> None:
        self.model = model
...
def setup_model_and_optimizer(
    ...
    if local_smoke:
        return LocalSmokeEngine(model, optimizer)
...
def write_storage_result(summary: dict[str, Any], run_id: str) -> dict[str, Any] | None:
    account_name = os.environ.get(
        "ANYSCALE_RESULTS_STORAGE_ACCOUNT",
        os.environ.get("ANYSCALE_PROOF_STORAGE_ACCOUNT", ""),
    ).strip()
    container_name = os.environ.get(
        "ANYSCALE_RESULTS_STORAGE_CONTAINER",
        os.environ.get("ANYSCALE_PROOF_STORAGE_CONTAINER", ""),
    ).strip()
    ...
    storage_path = f"{container_name}/anyscale/results/{run_id}/workload-summary.json"
    storage_uri = f"az://{storage_path}"
    abfss_uri = f"abfss://{container_name}@{account_name}.dfs.core.windows.net/anyscale/results/{run_id}/workload-summary.json"
    storage_result = {
    ...
    summary["storage_result"] = storage_result
    ...
    filesystem.makedirs(f"{container_name}/anyscale/results/{run_id}", exist_ok=True)
    ...
    if round_trip != payload:
        raise RuntimeError(
            "managed identity storage result round trip did not match written payload"
        )
...
def report_metrics_and_save_checkpoint(
    ...
    if context.get_world_rank() == 0:
        summary_path = summary_dir / "workload-summary.json"
        summary = {
            "run_id": run_id,
        ...
        write_storage_result(summary, run_id)
        save_summary(summary_path, summary)
        print(
            f"WORKLOAD_SUMMARY_JSON={json.dumps(summary, sort_keys=True, separators=(',', ':'))}",
            flush=True,
        )
...
def train_loop(config: dict[str, Any]) -> None:
    ...
    summary_dir = Path(config["results_dir"])
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AKS Flex deepspeed_finetune workload")
    parser.add_argument("--run-id", default=f"results-{uuid.uuid4().hex[:8]}")
    ...
    parser.add_argument("--results-dir", "--evidence-dir", dest="results_dir", default="./results")
    parser.add_argument(
        "--expected-regions",
        nargs="+",
        default=["westus3", "westus2"],
        help="Region labels expected in the workload results.",
    )
    parser.add_argument(
        "--placement-hint-env",
        default="AKS_NODE_REGION",
        help="Environment variable used to record the node region in workload results.",
    )
    parser.add_argument(
        "--storage-path",
        default="/tmp/anyscale-results-ray-train",
    ...
    parser.add_argument(
        "--local-smoke",
        action="store_true",
        help="Use native PyTorch for local validation instead of DeepSpeed runtime extensions.",
    )
    return parser
