...
variables {
  ...
  gpu_pool_configs = {
    GPU = {
      name               = "gpupool"
      vm_size            = "configured-gpu-vm-size"
      product_name       = "configured-gpu-product"
      gpu_count          = "1"
      min_count          = 1
      max_count          = 2
      availability_zones = []
...
run "foundation_plan_contract" {
  ...
  assert {
    condition     = length(module.aks.container_insights.streams) == 1 && module.aks.container_insights.streams[0] == "Microsoft-ContainerInsights-Group-Default"
    error_message = "Container Insights must use the supported grouped DCR stream."
  }
  ...
  assert {
    condition     = output.storage_account_name == "sttftestciwus3b602d5"
    error_message = "Storage account naming must include the stable subscription hash."
  }

  assert {
    condition     = output.acr_name == "crtftestciwus3b602d5"
    error_message = "ACR naming must include the stable subscription hash."
  }
