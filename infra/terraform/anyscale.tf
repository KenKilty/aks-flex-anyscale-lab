...
resource "null_resource" "anyscale_gateway_static_ip" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      AKS_CLUSTER_NAME   = self.triggers.aks_cluster_name
      AKS_RESOURCE_GROUP = self.triggers.aks_resource_group
      GATEWAY_NAME       = self.triggers.gateway_name
      GATEWAY_NAMESPACE  = self.triggers.gateway_namespace
    }
    ...
    command = <<-EOT
      ...
      while [ "$ATTEMPT" -le 60 ]; do
        ...
        if [ -z "$SERVICE_NAME" ]; then
          echo "Anyscale Gateway service released the Terraform-managed public IP."
          exit 0
        fi
      ...
      kubectl get svc \
        ...
        --selector "gateway.networking.k8s.io/gateway-name=$GATEWAY_NAME" \
        --output wide
      echo "Anyscale Gateway service did not release the Terraform-managed public IP."
      exit 1
    EOT
  }
