locals {
  ...
  global_hash = substr(sha1(var.azure_subscription_id), 0, 6)
  ...
  names = {
    ...
    storage_account            = "${substr("st${local.suffix_alt}", 0, 18)}${local.global_hash}"
    acr                        = "${substr("cr${local.suffix_alt}", 0, 44)}${local.global_hash}"
