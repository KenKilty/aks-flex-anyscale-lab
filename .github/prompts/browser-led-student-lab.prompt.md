...
- Use only commands, values, links, decisions, and recovery steps that a rendered student page presents. Do not use test harnesses, undocumented helpers, remembered commands, or a source-code search to make progress. This repository intentionally has no script that deploys the lab end to end; the rendered modules are the only deployment path.
